...
class IK:
    ...
    def joint_centering_task(self,q,rate=5e-1):
        """
        ...
        joints. Reducing this parameter improves convergence stability for the
        ...
        offset = 2 * (q - self.center) / (self.upper - self.lower)
    ...
    def direction_to_move(target, current):
        """
        Helper function for the End Effector Task. Computes an end effector
        velocity which will bring the end effector closer to the target position
        and orientation.

        INPUTS:
        target - 4x4 numpy array representing the desired transformation from
        end effector to world
        target - 4x4 numpy array representing the "current" end effector orientation

        OUTPUTS:
        v - a 1x3 numpy array containing the linear velocity with which to move
        the end effector to decrease the position error, expressed in the world frame
        omega - a 1x3 numpy array containing the angular velocity with which to
        move the end effector to decrease the orientation error, expressed in the world frame
        """
    ...
    def distance_and_angle(G, H):
        ...
        transforms. Can be used to decide whether two transforms can be
        considered the same to a certain linear and angular tolerance.
        ...
        angle - the magnitude in radians of the angle between the orientations
        of G & H
    ...
    def inverse(self, target, seed):
        ...
        Terminates according to step size and max iterations passed to constructor.
...
if __name__ == "__main__":
    ...
    ik = IK() # uses default solver parameters
